    ...
    <>
      ...
      <div className="w-full flex flex-col">
        <label className="flex justify-between text-sm font-medium text-text-third mb-4">
          <span>Height</span>
          <span className="text-main-color">{height ? height : ""}</span>
        </label>
        <input
          min="150"
          max="250"
          className={cn(
            ...
            height && "disabled-slider cursor-default"
          )}
          type="range"
      ...
      <div className="w-full flex flex-col">
        ...
        <input
          ...
          className={cn(
            ...
            currentWeight && "disabled-slider cursor-default"
          )}
          type="range"
          defaultValue={currentWeight ? currentWeight : ""}
      ...
      <div className="w-full flex flex-col">
        ...
        <input
          ...
          className={cn(
            "w-full h-2 bg-tertiary-dark rounded-lg appearance-none cursor-pointer accent-main-color-hover mb-2",
            goalWeight && "disabled-slider cursor-default"
          )}
          type="range"
          defaultValue={goalWeight ? goalWeight : ""}
          disabled={!!goalWeight}
        />
        <div className="flex justify-between text-xs text-slate-600">
          <span>40 kg</span>
      ...
      <div className="w-full flex flex-col ">
        ...
        <div className="w-full flex items-center justify-between gap-3 ">
          {["Sedentary", "Moderate", "Active"].map((level) => {
            const isActive = activity && activity === level ? true : false;
            return (
              <Button
                key={level}
                className={cn(
